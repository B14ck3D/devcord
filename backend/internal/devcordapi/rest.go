...
func (a *App) handleListMembers(w http.ResponseWriter, r *http.Request) {
	...
	memberRows, err := a.pool.Query(ctx, `
		SELECT u.id, u.display_name, u.nick,
			COALESCE(
				(SELECT string_agg(mr.role_id::text, ',' ORDER BY r.position DESC)
				 FROM member_roles mr JOIN roles r ON r.id = mr.role_id
				 WHERE mr.user_id = u.id AND mr.server_id = sm.server_id),
				''
			) AS role_ids_csv,
			COALESCE(
	...
	for memberRows.Next() {
		...
		if memberRows.Scan(&uID, &dn, &nick, &roleCSV, &rid, &aURL, &nColor, &nGlow) != nil {
			continue
		}
		var roleIDList []string
		if strings.TrimSpace(roleCSV) != "" {
			for _, p := range strings.Split(roleCSV, ",") {
				p = strings.TrimSpace(p)
				if p != "" {
					roleIDList = append(roleIDList, p)
				}
			}
		}
		ridStr := ""
		...
		if len(roleIDList) == 0 && ridStr != "" {
			roleIDList = []string{ridStr}
		}
		if len(roleIDList) > 0 {
			ridStr = roleIDList[0]
		}
		mMap := map[string]interface{}{
			"id": strconv.FormatInt(uID, 10), "name": dn, "nick": nick, "roleId": ridStr, "role_ids": roleIDList, "status": "online",
		}
		if aURL != nil {
