...
type ChatHub struct {
	...
	dm      map[int64]map[*wsConn]struct{}
	app     *App
	typMu   sync.Mutex
	lastTyp map[string]time.Time
	lastDmTyp map[string]time.Time
}

func newChatHub() *ChatHub {
	return &ChatHub{
		ch:        make(map[int64]map[*wsConn]struct{}),
		dm:        make(map[int64]map[*wsConn]struct{}),
		lastTyp:   make(map[string]time.Time),
		lastDmTyp: make(map[string]time.Time),
	}
}
...
func (h *ChatHub) unsubscribeAll(c *wsConn) {
	...
	for convID := range c.dmConversations {
		if m, ok := h.dm[convID]; ok {
			delete(m, c)
			if len(m) == 0 {
				delete(h.dm, convID)
			}
	...
	for k := range c.dmConversations {
		delete(c.dmConversations, k)
	}
}
...
func (h *ChatHub) subscribeDm(convID int64, c *wsConn) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.dm[convID] == nil {
		h.dm[convID] = make(map[*wsConn]struct{})
	}
	h.dm[convID][c] = struct{}{}
	c.dmConversations[convID] = struct{}{}
}

func (h *ChatHub) BroadcastDm(convID int64, msg []byte) {
	h.mu.RLock()
	m := h.dm[convID]
	h.mu.RUnlock()
	for c := range m {
...
func (h *ChatHub) maybeTypingDm(convID, uid int64, typing bool) {
	key := "dm:" + strconv.FormatInt(convID, 10) + ":" + strconv.FormatInt(uid, 10)
	now := time.Now()
	h.typMu.Lock()
	if typing {
		if t, ok := h.lastDmTyp[key]; ok && now.Sub(t) < 3*time.Second {
			h.typMu.Unlock()
			return
		}
		h.lastDmTyp[key] = now
	} else {
		delete(h.lastDmTyp, key)
	}
	h.typMu.Unlock()
	payload, err := json.Marshal(map[string]interface{}{
		"type": "typing",
