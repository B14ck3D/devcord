...
type wsConn struct {
	...
	dmConversations map[int64]struct{}
	uid             int64
	writeMu         sync.Mutex
}
...
func (a *App) handleChatWS(w http.ResponseWriter, r *http.Request) {
	...
	wc := &wsConn{
		...
		dmConversations: make(map[int64]struct{}),
		uid:             uid,
	}
	go wc.writePump()
	wc.readPump()
...
// Client → server: subscribe (channel), subscribe_dm (conversation), typing, typing_dm.
// Server → client: message (channel_id | conversation_id), typing, user_updated.
...
func wsJSONIDString(v interface{}) string {
	switch x := v.(type) {
	case nil:
		return ""
	case string:
...
func (wcc *wsConn) readPump() {
	...
	for {
		...
		switch m.Type {
		case "subscribe":
			chID, err := strconv.ParseInt(wsJSONIDString(m.ChannelID), 10, 64)
			if err != nil {
				cancel()
				continue
			}
		...
		case "subscribe_dm":
			convID, err := strconv.ParseInt(wsJSONIDString(m.ConversationID), 10, 64)
			if err != nil {
				cancel()
				continue
			}
			ok, err := wcc.app.dmMember(ctx, wcc.uid, convID)
			cancel()
			if err != nil || !ok {
				continue
			}
			wcc.hub.subscribeDm(convID, wcc)
		case "typing_dm":
			convID, err := strconv.ParseInt(wsJSONIDString(m.ConversationID), 10, 64)
			if err != nil {
				cancel()
			...
			wcc.hub.maybeTypingDm(convID, wcc.uid, t)
		default:
			cancel()
		}
	}
