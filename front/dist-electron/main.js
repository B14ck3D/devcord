...
app.commandLine.appendSwitch('enable-features', 'WebRtcHideLocalIpsWithMdns,WebRtcApmInAudioService,AudioWorkletRealtimeThread');
...
async function prepareIsolatedSidecarBinary(sourcePath) {
    const localAppData = process.env.LOCALAPPDATA?.trim();
    if (!localAppData) {
...
function setupDesktopSessionNetworking() {
    const installMediaPermissions = (ses) => {
...
function setupIpc() {
    ...
    ipcMain.handle('devcord:updater-install-now', async () => {
        ...
        try {
            ...
            if (!sidecarPath) {
                throw new Error('Nie znaleziono sidecar updatera (oczekiwano pliku DevcordInstaller/Devcord_Installer).');
            }
            ...
            const child = spawn(isolatedSidecarPath, ['--update-mode', `--archive-path=${updateZipPath}`], {
                detached: true,
                stdio: 'ignore',
            });
            child.unref();
